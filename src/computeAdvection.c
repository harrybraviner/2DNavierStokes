...
void computeAdvection(struct Field *fld){
	...
	for(i=0; i<NTOTAL_REAL; i++){	// Loop over spatial points FIXME - is this actually looping over the correct points? See FFTW3 manual
	...
	for(i=0; i<NTOTAL_COMPLEX; i++){
		...
		fld->Axx[i] /= (NX*NY);
		fld->Ayy[i] /= (NX*NY);
		fld->Axy[i] /= (NX*NY);	// I think this is right, based on FFTW3 manual.
	}
}
